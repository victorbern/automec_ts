...
describe("Find All Pagamentos", () => {
    ...
    const findAllPagamentos = new FindAllPagamentosUC(mySqlPagamentos, findPagamento);

    const data = new Date(Date.now());

    // Limpa os dados de pagamento para teste
    mySqlPagamentos.items = [];

    // Cria dois pagamentos para teste
    ...
    it("should be able to find all payments", () => {
        expect(findAllPagamentos.execute({})).resolves.toStrictEqual([
            {
                idPagamento: 2,
                subtotal: 20,
                total: 40,
    ...
    it("should not be able to find any payment", () => {
        // Limpa todos os dados de pagamento para teste
        mySqlPagamentos.items = [];

        expect(findAllPagamentos.execute({})).resolves.toHaveLength(0);
    })
